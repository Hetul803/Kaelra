"""Real Google API readers (Calendar, Gmail, Drive).

Each function returns data in the SAME shape the mock connectors produce, so the
connector layer can transparently swap real <-> mock. Used only when the user
    ...
    try:
        ...
        for e in res.get("items", []):
            ...
            events.append({"id": e.get("id"), "title": title, "start": _fmt_time(s) if s else "",
                           ...
                           "location": e.get("location", ""), "kind": kind})
...
async def drive_file_text(user_id: str, file_id: str) -> tuple[str, str]:
    """Return (filename, extracted text) for a Drive file."""
    creds = await get_credentials(user_id)
    if not creds:
        return "", ""
    try:
...
async def create_gmail_draft(user_id: str, to: str, subject: str, body: str) -> str | None:
    """Create a Gmail DRAFT (never auto-send). Returns draft id or None."""
    creds = await get_credentials(user_id)
    if not creds:
        return None
    try:
